...
- [1. 基础](### 1. 基础)
  - [闭包原理](####1. 闭包原理)
  - [原型，原型链，继承有哪些](#### 2. 原型，原型链，继承有哪些)
  - [事件代理](#### 3. 事件代理)
  - [事件注册](#### 4. 事件注册)#### 4. 事件注册
  - [js 对象继承有哪些方式](#### 5. js 对象继承有哪些方式)
  - [双向绑定的原理](#### 6. 双向绑定的原理)

- [进阶](### 2. 进阶)
  - [web workers的理解](#### 7. web workers的理解)
  - [设计一个布局方案，使得页面在pc和pad端显示为一行三列，在于手机端为一列三行](#### 8. 设计一个布局方案，使得页面在pc和pad端显示为一行三列，在于手机端为一列三行)   
  - [自定义一个浏览器兼容的事件绑定方法需要注意哪些问题](#### 9. 自定义一个浏览器兼容的事件绑定方法需要注意哪些问题)
  - [跨域有哪些，以及它们之间的区别]()#### 10. [跨域有哪些，以及它们之间的区别](https://github.com/sakila1012/font-end-interview-question/issues/3)
  - [js中也会有排序的需求，用js实现一个标准的排序方法，对某个数字数组进行由低到高排序](#### 11. js中也会有排序的需求，用js实现一个标准的排序方法，对某个数字数组进行由低到高排序)
  - [将多维数组转换为一维数组，比如[1,[2,3,[5,6,[7,8]]]],转换为[1,2,3,4,5,6,7,8]](#### 12. 将多维数组转换为一维数组，比如[1,[2,3,[5,6,[7,8]]]],转换为[1,2,3,4,5,6,7,8])
  - [创建一个js类，模拟实现方法的重载](#### 13. 创建一个js类，模拟实现方法的重载)
  - [请根据以下提供的HTML，实现一个图片自动轮图，每3秒切换一张图片，循环播放。（允许使用jQuery）](#### 14. 请根据以下提供的HTML，实现一个图片自动轮图，每3秒切换一张图片，循环播放。（允许使用jQuery）)
  - [设计并实现一个combox（下拉选择框）（携程2017机试](#### 15. 设计并实现一个combox（下拉选择框）（携程2017机试）)  
  1、combox内容可编辑，下拉区域可滚动（一共包含了10W条数据）  
  2、根据输入的内容做prefix匹配，展示匹配的可选择内容列表（要求提高匹配效率，考察算法能力，空间复杂度和时间复杂度）
  - [你知道哪些JS的第三方库或框架，请举例说明，并阐述他们有哪些优点？](### 16. 你知道哪些JS的第三方库或框架，请举例说明，并阐述他们有哪些优点？)
  - [浏览器兼容问题有哪些，怎么解决的](### 17. 浏览器兼容问题有哪些，怎么解决的)

- [计算机网络](### 3. 计算机网络)
  - [进程是资源分配的最小单位，线程是CPU调度的最小单位](#### 3.1 进程是资源分配的最小单位，线程是CPU调度的最小单位)
  - [HTTP状态码](#### 3.2 HTTP状态码)
  - [HTTP协议使用的端口号](#### 3.3 HTTP协议使用的端口号)
  - [无线网络和有线网络在osi结构从上到下从从哪一层开始不一样的？](#### 3.4 无线网络和有线网络在osi结构从上到下从从哪一层开始不一样的？)
  - [UDP伪首部](#### 3.5 UDP伪首部)

- [数据库](### 4. 数据库)
  - [ 二叉树](#### 4.1  二叉树)
  - [一个完全二叉树有770个节点，那么它的叶子节点数共有多少个？](#### 4.2 一个完全二叉树有770个节点，那么它的叶子节点数共有多少个？) 
  - [ K-means 的原理，有哪些输入参数](#### 4.3 K-means 的原理，有哪些输入参数)

- [其他](### 5. 其他)
  - [SQL注入](#### 5.1 SQL注入)
  - [Oauth2](#### 5.2 Oauth2)
  - [说说你熟悉的设计模式](#### 5.3 说说你熟悉的设计模式)(https://github.com/FrankKai/FrankKai.github.io/issues/3)
  - [说一下你理解的模块机制](#### 5.4 说一下你理解的模块机制)
  - [MVVM 原理](#### 5.5 MVVM 原理)
  - [最熟悉框架的路由机制](#### 5.6 最熟悉框架的路由机制)
  - [状态管理](#### 5.7 状态管理)
  - [统计字符串中单词出现的次数](#### 5.8 统计字符串中单词出现的次数)
  - [平时遇到的跨域问题](#### 5.9 平时遇到的跨域问题)
  - [下面这段代码的输出](#### 5.10 下面这段代码的输出)
  - [["1","2","3"].map(parseInt)返回的是什么？](#### 5.11 ["1","2","3"].map(parseInt)返回的是什么？)
  - [下面代码中“入库”的颜色是？](#### 5.12 下面代码中“入库”的颜色是？)
  - [谈谈个人理想中的团队](#### 5.13 谈谈个人理想中的团队)
  - [个人成长空间](#### 5.14 个人成长空间)
  - [公司对员工的成长和进步的重视程度](#### 5.15 公司对员工的成长和进步的重视程度)
  - [加班情况](#### 5.16 加班情况)
  - [员工补贴，福利](#### 5.17 员工补贴，福利)
  - [绩效考核和年终奖](#### 5.18 绩效考核和年终奖)

- [算法](### 6. 算法)
  - [递归](#### 6.1 [递归])#### 6.1 [递归](https://github.com/sakila1012/font-end-interview-question/issues/2)
